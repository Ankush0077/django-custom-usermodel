...
class CustomUserManager(BaseUserManager): 
    def create_user(self,phone_number,current_otp,**other_fields): 
        if not phone_number:
            raise ValueError(_("You must provide a phone number"))
        if not current_otp:
            raise ValueError(_("You must provide a otp"))
        
        user=self.model(phone_number=phone_number,current_otp=current_otp,**other_fields)
        user.set_password(current_otp)
        user.save()
        return user
    
    def create_superuser(self,phone_number,current_otp,**other_fields):
